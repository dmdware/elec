...
v4f gfpv4[8];
v4f gfpl4[6];
v4f gfxax, gfyax, gfzax;
v4f gfview;
v4f gfpos;
float gfwh;

v4f(*toclipvp[4])(v4f vi) = {toclipvp0, toclipvp1, toclipvp2, toclipvp3};
...
{
	float d[12];
	return toxy2(vi, 1, 1, gfpv, gfpl, gfpld, d, gfsa);
}

v4f toclipvp0(v4f vi)
{
	v4f vo;
	float extx = PROJ_RIGHT*gfwh / 2.0f;
	float exty = PROJ_RIGHT / 2.0f;
	vo.x = (vi.x - gfview.x + extx / 2.0f) / extx;
	vo.y = (vi.y - gfview.y + exty / 2.0f) / exty;
	vo.z = (vi.z - gfpos.z + MIND) / (MAXD-MIND);
	vo.w = 1;
	return vo;
}
v4f toclipvp1(v4f vi)
{
	...
	float extx = PROJ_RIGHT*gfwh / 2.0f;
	float exty = PROJ_RIGHT / 2.0f;
	vo.x = (vi.w - gfview.w + extx / 2.0f) / extx;
	vo.y = (vi.y - gfview.y + exty / 2.0f) / exty;
	vo.z = (vi.x - gfpos.x + MIND) / (MAXD - MIND);
	vo.w = 1;
	return vo;
}
v4f toclipvp2(v4f vi)
{
	...
	float exty = PROJ_RIGHT / 2.0f;
	vo.x = (vi.x - gfview.x + extx / 2.0f) / extx;
	vo.y = (vi.z - gfview.z + exty / 2.0f) / exty;
	vo.z = (vi.w - gfpos.w + MIND) / (MAXD - MIND);
	vo.w = 1;
	return vo;
}
v4f toclipvp3(v4f vi)
{
	...
	vo.x = (vi.w - gfview.w + extx / 2.0f) / extx;
	vo.y = (vi.z - gfview.z + exty / 2.0f) / exty;
	vo.z = (vi.y - gfpos.y + MIND) / (MAXD - MIND);
	vo.w = 1;
	return vo;
}
