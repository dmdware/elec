...
static mpz_t  *sq_res_0x100;          /* table of limbs */
static int    nsq_res_0x100;          /* elements in sq_res_0x100 array */
static int    sq_res_0x100_num;       /* squares in sq_res_0x100 */
static double sq_res_0x100_fraction;  /* sq_res_0x100_num / 256 */

static int     mod34_bits;        /* 3*GMP_NUMB_BITS/4 */
static int     mod_bits;          /* bits from PERFSQR_MOD_34 or MOD_PP */
static int     max_divisor;       /* all divisors <= max_divisor */
static int     max_divisor_bits;  /* ceil(log2(max_divisor)) */
static double  total_fraction;    /* of squares */
static mpz_t   pp;                /* product of primes, or 0 if mod_34lsub1 used */
static mpz_t   pp_norm;           /* pp shifted so NUMB high bit set */
static mpz_t   pp_inverted;       /* invert_limb style inverse */
static mpz_t   mod_mask;          /* 2^mod_bits-1 */
static char    mod34_excuse[128]; /* why mod_34lsub1 not used (if it's not) */

/* raw list of divisors of 2^mod34_bits-1 or pp, just to show in a comment */
static struct rawfactor_t {
  int     divisor;
  int     multiplicity;
};
static struct rawfactor_t  *rawfactor;
static int                 nrawfactor;

/* factors of 2^mod34_bits-1 or pp and associated data, after combining etc */
static struct factor_t {
  int     divisor;
  mpz_t   inverse;   /* 1/divisor mod 2^mod_bits */
  mpz_t   mask;      /* indicating squares mod divisor */
  double  fraction;  /* squares/total */
};
static struct factor_t  *factor;
static int              nfactor;       /* entries in use in factor array */
static int              factor_alloc;  /* entries allocated to factor array */
